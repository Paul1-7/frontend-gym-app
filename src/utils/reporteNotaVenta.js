...
function jsPDFInvoiceTemplate(props) {
  ...
  // const startPointRectPanel1 = currentHeight + 6;
  ...
  if (param.invoice.header.length > 2) {
    ...
    const customWidthOfAllColumns = customColumnNo.reduce((a, b) => a + b, 0);
    tdWidth =
  ...
  param.invoice.table.forEach((row, index) => {
    doc.line(10, currentHeight, docWidth - 10, currentHeight);
  ...
  const invDescSize = splitTextAndGetHeight(param.invoice.invDesc, docWidth / 2).height;
